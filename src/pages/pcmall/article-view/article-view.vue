...
export default {
  ...
  async mounted() {
    ...
    // 发请求
    // SEO
    let seo = await getSpecialSeo({id: id});
    if (seo.data) {
      this.channelMark = seo.data.channelmark;
      document.querySelector('title').innerHTML = seo.data.title;      
    }

    // 商品信息
    let product = await getSpecialProductList({id: id});
    if (product && product.productList) {
      this.productList = product.productList;
    }

    // 案例 + 模块
    let specialCase = await getSpecialCase({id: id});
    console.log(specialCase);

    let specialContent = await getSpecialContent({id: id});
    specialContent.contentdata && (this.contentdata = entityToString(specialContent.contentdata));
